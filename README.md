...
● 怎么通过fork复制出一个进程？怎么通过exec加载一个用户程序的elf可链接执行文件创建一个用户进程？<br>
● 段页式映射机制是怎么工作的？<br>
● 虚存管理是如何给用户程序提供了一个磁盘大小的内存空间的假象？<br>
● 函数调用怎么创建栈帧？<br>
● 一个进程的虚拟地址空间分布是什么样的？<br>
● ......<br>
...
## 段页式映射机制 
Ucore是基于80386的CPU架构，CPU地址线有32位，可以寻址4GB的物理内存空间。在Ucore中是采用了段页式的映射机制（包括Linux中也是段页式的映射机制，不过段机制对大量应用程序分散地使用大内存的支持能力较弱，所以需要使用页机制）。Ucore和Linux中都弱化了段式映射，段式映射都采用固定地址映射。<br>
在Ucore中，包含三类地址：逻辑地址，线性地址，物理地址。有两个寄存器来保存地址值，一个16位的CS（code segment代码段）寄存器用来选择相应的段进行段式映射，还有一个32位的eip指令寄存器。具体来说，映射过程如下：<br>

1. 逻辑地址经过段式映射转换为线性地址：16位CS寄存器作为段选择子，从GDT全局描述符表中查找得到对应的段的段描述符。段描述符中保存了该段的基址（32位），将段的基址与eip指令寄存器的32位地址拼接，得到一个32位的线性地址。
2. 线性地址经过页式映射转换为物理地址：32位线性地址通过2级页表映射转换为最终的物理地址
...
1. CPU在执行完一条指令后从总线上读取中断请求的中断向量，以该向量为索引到IDT中断描述符表中找到该向量对应的中断描述符，中断描述符中保存着中断服务例程的段选择子
2. 利用查到的中断服务例程的段选择子从GDT中找到对应的段描述符，段描述符记录了中断服务例程的段基址信息。并且中断描述符中记录了中断服务例程的32为偏移地址。根据段基址和偏移地址，最终就能找到中断服务例程所处的物理地址。
3. 发生中断时，CPU会负责保存被打断程序的现场，会将eflags，cs，eip，errocode寄存器压入到栈中，ucore负责把通用寄存器压入栈中。
4. 如果中断时的程序处于用户态级别，那么就会发生CPU特权级的切换，从用户态切换到内核态，除此之外还要将用户栈切换为内核栈。

在发生中断时，CPU会自动根据IDTR寄存器找到中断描述符表，根据中断向量进行中断处理程序的查找和地址翻译，并且会帮助我们进行一些寄存器的保存压栈。那么OS需要做的事情就是建立好中断描述符表IDT，维护好每个中断处理程序所处的地址，再将IDT表的起始物理地址存入到IDTR中。

## 物理内存管理
在bootloader程序中，通过一些机器指令（e820h中断）完成了物理内存的探测，我们需要通过创建Page结构体来管理实际的4KB物理页，Page结构体的定义如下：
```
struct Page {
    int ref;                     //当前物理页被多少虚拟地址引用
    uint32_t flags;              //页的状态（比如是空闲页，还是已分配）
...
First-Fit物理页分配算法实现，从头遍历双向链表，找到第一个满足分配需求的空闲块。
```
